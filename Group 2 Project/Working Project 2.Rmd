...
```{r}
##Attach the monthly data and cluster assignment, AND calculate annual sums for summary purposes, since the dedicated annual columns were not found.
df_clustered <- df_cleaned %>%
  mutate(
    cluster = as.factor(kmeans_result$cluster),
    
    ##Calculate Annual Net Generation by summing monthly MWh columns
    calculated_annual_net_gen = rowSums(
      clustering_data %>% select(starts_with("electricity_net_generation_m_wh_")),
      na.rm = TRUE
    ),
    
    ##Calculate Annual Quantity Consumed by summing monthly quantity columns
    calculated_annual_quantity_consumed = rowSums(
      clustering_data %>% select(starts_with("quantity_")),
      na.rm = TRUE
    )
  )
```
...
##NOW USING THE CALCULATED ANNUAL COLUMNS
...
##Using Principal Component Analysis to reduce the 24 monthly features down to 2 components so we can plot the clusters on a 2D graph.
...
problem_data_points <- df_clustered %>%
  ...
  ##Using the calculated annual columns here
